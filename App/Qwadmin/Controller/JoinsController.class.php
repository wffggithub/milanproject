...
class joinsController extends ComController
{
    ...
    public function index($sid = 0, $p = 1)
    {
        ...
        if ($sid) {
            ...
            $where .= "and {$prefix}withus.sid in ($sids) ";
        }
        if ($keyword) {
            $where .= "and {$prefix}withus.title like '%{$keyword}%' ";
        }
        //默认按照时间降序
        $orderby = "t asc";
        if ($order == "desc") {
        ...
        $count = $withus->where($where)->count();
        $list = $withus->field("{$prefix}withus.*,{$prefix}category.name")->where($where)->order($orderby)->join("{$prefix}category ON {$prefix}category.id = {$prefix}withus.sid")->limit($offset . ',' . $pagesize)->select();
    ...
    public function edit($id)
    {
        ...
        $id = intval($id);
        $withus = M('withus')->where('id=' . $id)->find();
        if ($withus) {
           ...
           /* $tree = new Tree($category);
            $str = "<option value=\$id \$selected>\$spacer\$name</option>"; //生成的形式
            $category = $tree->get_tree(0, $str, $withus['sid']);*/
            $this->assign('category', $category);//导航

            $this->assign('withus', $withus);
        } else {
            $this->error('参数错误！');
        }
        $this->display('form');
    ...
    public function update($id = 0,$act = null)
    {
        if ($act == 'order') {
            ...
            $o = I('post.o', 0, 'intval');
            M('withus')->data(array('o' => $o))->where("id='{$id}'")->save();
            die('1');
        }
       ...
       */
       ...
       /* if (!$data['sid'] or !$data['title'] or !$data['content']) {
        ...
        if ($id) {
            M('withus')->data($data)->where('id=' . $id)->save();
           /* addlog('编辑文章，AID：' . $id);*/
            $this->success('恭喜！编辑成功！',U('index'));
        } else {
            $id = M('withus')->data($data)->add();
            if ($id) {
                /*addlog('新增文章，AID：' . $aid);*/
